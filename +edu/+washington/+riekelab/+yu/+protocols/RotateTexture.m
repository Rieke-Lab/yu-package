classdef RotateTexture < edu.washington.riekelab.protocols.RiekeLabStageProtocol
    ...
    methods
        ...
        function prepareRun(obj)
            ...
            %stimSize_VHpix = round(stimSize ./ (3.3)); %um / (um/pixel) -> pixel
        ...
        function p = createPresentation(obj)
            ...
            elseif strcmp(obj.displayMode,'continuous')
               ...
               % display((total_stimtime+obj.preTime)*1e-3);
                ...
                p.addStimulus(scene);
                sceneVisible = stage.builtin.controllers.PropertyController(scene, 'visible', ...
                     @(state)state.time >= obj.preTime * 1e-3 && state.time < (obj.preTime + total_stimtime) * 1e-3);
                p.addController(sceneVisible);
                sceneMatrix = stage.builtin.controllers.PropertyController(scene,'imageMatrix',...
                     @(state)getSceneMatrix(obj.centerTexture, state.time - obj.preTime/1e3, angular_vel));
                 p.addController(sceneMatrix);
            end
            function p = getSceneMatrix(img, time, vel)
                angle = time*vel;
                p = imrotate(img, angle, 'bilinear','crop');
            end
