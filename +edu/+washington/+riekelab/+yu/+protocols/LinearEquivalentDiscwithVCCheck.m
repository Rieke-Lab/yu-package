...
% check contrast responses periodically 
    properties
        ...
        noPatche_interval = 30 % after these image patches test discs will be shown
    ...
    methods
        ...
        function prepareRun(obj)
            ...
            if strcmp(obj.patchSampling,'random')
                %get patch indices:
                pullInds = randsample(1:length(xLoc),obj.numberOfAverages);
            else strcmp(obj.patchSampling,'ranked')
                %pull more than needed to account for empty bins at tail
                [~, ~, bin] = histcounts(responseDifferences,1.5*obj.numberOfAverages);
                populatedBins = unique(bin);
                %pluck one patch from each bin
                pullInds = arrayfun(@(b) find(b == bin,1),populatedBins);
                %get patch indices:
                pullInds = randsample(pullInds,obj.numberOfAverages);
            end
            obj.patchLocations(1,1:obj.numberOfAverages) = xLoc(pullInds); %in VH pixels
            obj.patchLocations(2,1:obj.numberOfAverages) = yLoc(pullInds);
