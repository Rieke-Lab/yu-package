classdef RevLinearEquivalentDisc < edu.washington.riekelab.protocols.RiekeLabStageProtocol
    ...
    properties (Hidden)
        ...
        waveformType = symphonyui.core.PropertyType('char','row',{'sine','square'})
            ...
            elseif strcmp(obj.stimulusTag,'intensity')
                ...
                if(obj.temporalFrequency>0)
                 sceneColor = stage.builtin.controllers.PropertyController(scene,'color',...
                     @(state)getSceneColor(equiIntensity, state.time - obj.preTime/1e3,background,freq, waveform_flag, iniphase));
