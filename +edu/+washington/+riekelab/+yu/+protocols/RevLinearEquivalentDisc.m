classdef RevLinearEquivalentDisc < edu.washington.riekelab.protocols.RiekeLabStageProtocol
    ...
    properties (Hidden)
        ...
        waveformType = symphonyui.core.PropertyType('char','row',{'sine','square','phase'})
...
%             imagesc(obj.imagePatchMatrix); colormap(gray); axis image; axis equal;
        ...
        function p = createPresentation(obj)
            ...
            if strcmp(obj.waveform,'sine')
                waveform_flag = 1;
            elseif strcmp(obj.waveform,'square')
                waveform_flag =0;
            ...
            function p = getSceneMatrix(img, time, b,f,w,p)
                % alternate image intensity
                     if time>0
                        ...
                        else % treat the image as pixels in different phases; with A = b;
                            contra = cos(time*f*pi*2+acos((img-b)/b));  
                        end
                        p = contra*b+b;
                     else p = img;
                     end
            ...
            function p = getSceneColor(intensity,time, b, f,w,p,img,weight)
                     if time >0
                     if (w == 0)
                            %square wave
                            if cos(time*f*pi*2+p)>0
                     ...
                     else % only shift phase
                         contra = sum(sum(cos(time*f*pi*2+acos((img-b)/b)).*weight));
                     end
                     p = contra*b+b;
                     else p = intensity;
                     end
